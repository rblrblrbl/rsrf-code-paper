...
if(!onCluster){
  setwd("C:/Users/ricar/Documents/Universitaet-Projekte/RSRF_revision_code")
  data_id_from_input <- "robot_small" #e.g. robot / robot_small
  alg_from_input <- "intf_norep"  #rf #et
  ntrees_from_input <- 17 #500
  replace_from_input <- TRUE
  seed_from_input <- 21  #e.g. 123, or "NA"
  round_from_input <- "both"
  seed <- seed_from_input
  submit_dir <- getwd()
  job_id <- "local"
  run_with_best_from_input <- TRUE #e.g. TRUE / FALSE
  outer_fold_number_from_input <- 3 #e.g. 1,2,3,4,5
  round_id_from_input <- "round2" #e.g. round1 or round2
  directory <- "C:/Users/ricar/Documents/Universitaet-Projekte/RSRF_revision_code"
...
#Read files
  ...
  if( alg_value %in% c("intf","intf_norep")){
    ...
    if( data_id %in% c("concrete","airfoil","abalone","robot","abalone_fm", "abalone_enc")){
      npairs_values_inner <- c(1,10,seq(25,250,by=25))
      npairs_values_inner_length <- length(npairs_values_inner)
    } else if (data_id %in% c("concrete_hd","airfoil_hd", "abalone_hd", "robot_hd", "chd", "abalone_fm_hd", "abalone_enc_hd")){
      npairs_values_inner <- c(1,10,seq(25,250,by=25),seq(300,700,by=100))
      npairs_values_inner_length <- length(npairs_values_inner)
    } else if (data_id %in% "chd_hd"){
      npairs_values_inner <- c(50, seq(100,1000,by=100) )
      npairs_values_inner_length <- length(npairs_values_inner)
    } else if (data_id %in% "robot_small"){
      npairs_values_inner <- c(2,3,4,5)
      npairs_values_inner_length <- length(npairs_values_inner)
    } else{
    ...
    res_matrix <- foreach( jinner = icount( nfolds_inner ),  .combine = 'cbind', .options.multicore=mcoptions  ) %:%
