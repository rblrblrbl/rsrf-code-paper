#File for running RSRF on real dataset using nested CV
...
if(onCluster){
  ...
  data_id_from_input <- as.character(args[[1]]) #e.g. airfoil / robot_small
  is_test_from_input <- as.logical(args[[2]]) #e.g. FALSE
  width_from_input <- as.integer( args[[3]]) #e.g. 5
  alg_from_input <- as.character( args[[4]])  #rsrf_nf or rsrf_nf_norep
  ...
  round_id <- as.integer( args[[8]] ) #1 or 2
  if( round_id == 1 ){
    round_id <- "_round1"
...
if( data_id %in% c("concrete","airfoil","abalone_enc","robot","chd")){
  mtry_values_inner <- 1:num_covariable
  mtry_values_inner_length <- length(mtry_values_inner)
} else if (data_id %in% c("concrete_hd","airfoil_hd", "abalone_hd", "abalone_enc_hd", "robot_hd")){
  mtry_values_inner <- c(10,20,30,40,50,num_covariable)
  mtry_values_inner_length <- length(mtry_values_inner)
} else if (data_id %in% "robot_small"){ #only used for testing purposes
...
if( alg_value %in% c("rsrf_af","rsrf_af_norep" ) ){
  stop("This file is for simulation for rsrf_nf/rsrf_nf_norep.")
}

if( alg_from_input %in% c("rsrf_nf","rsrf_nf_norep") ){
  mtry_mode_value <- "nothingfixed"
  include_CART_CART_value <- TRUE
  fixed_cart_cart_value <- FALSE
  mtry_cart_cart_value <- "to_tune"
  mtry_rsrf_step_value <- "to_tune"
  mtry_rsrf_step_random_value <- NULL
  if( alg_from_input == "rsrf_nf"){ replace_value <- TRUE }
  if( alg_from_input == "rsrf_nf_norep"){ replace_value <- FALSE }
}
...
### for reproducability
### Following chapter 5.2 of
### https://cran.r-project.org/web/packages/doRNG/vignettes/doRNG.pdf (work around for nested foreach loops)
### (as of 30/12/2024)
if( onCluster){
...
if(onCluster){
ptime <- system.time({
  res_matrix <- foreach(j = icount(nfolds_inner), .combine = 'cbind' ) %:% #outer loop mit cbind: column
    foreach( m_index = icount(mtry_values_inner_length), r=rng[(j-1)*mtry_values_inner_length + 1:mtry_values_inner_length], .combine='c') %dopar% { #inner loop wird dann zeile
      ...
      parameter_list_current$mtry_cart_cart$value <- mtry_values_inner[m_index] #for rsrf-nothing-fixed mode
      parameter_list_current$mtry_rsrf_step$value <- mtry_values_inner[m_index] #for rsrf-nothing-fixed mode
...
parameter_list_best$mtry_cart_cart$value <- mtry_best
