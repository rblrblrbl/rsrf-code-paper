#File for running nested CV for ET on real dataset
library(ranger)
library(foreach)
library(iterators)
library(ranger)
...
for( round in round_vals){
  ...
  if( alg_value %in% c("et_replace","et_sf1") ){
    ...
    if( data_id %in% c("concrete","airfoil","abalone","robot","chd","abalone_fm")){
      mtry_values_inner <- 1:num_covariable
      nsplits_values_inner <- 1:10
    } else if (data_id %in% c("concrete_hd","airfoil_hd", "abalone_hd", "robot_hd", "chd_hd", "abalone_fm_hd")){
  ...
  tune_values_chosen <- list() #saves chosen tuning parameter value per outer fold
  ...
  for( jouter in 1:nfolds_outer){
    ...
    tune_values_chosen[[jouter]] <- list( mtry_best = mtry_best, nsplits_best = nsplits_best)
    
  }
  
  df_all_outerfolds <- do.call( rbind, lapply( tune_values_chosen, function(x) as.data.frame(x) ) )
  rownames(df_all_outerfolds) <- paste0("outerfold_",1:nfolds_outer)
  
  saveRDS( list(
    round_id = round,
    mse_errors_outer = mse_errors_outer,
    tune_values_chosen = df_all_outerfolds),
  ...
  cat("Results from CV for ET\n\n",
      ...
      "nsplits:       ", df_all_outerfolds$nsplits_best,"\n",
      "mtry values:   ", df_all_outerfolds$mtry_best, "\n\n",
