...
#SBATCH --job-name=af_cv_F6_1-10
#SBATCH --mail-type=ALL
#SBATCH --output=rsrf_cv_af.%j.txt
...
# Start program
R CMD BATCH --no-save --no-restore --slave '--args 1 10 F 6 478 FALSE' rsrf_cv_server_af_slurm.R


#Copy back results after your application finished
cp rsrf_cv_server_af_slurm.Rout $RESULTDIR/rsrf_cv_server_${SLURM_JOB_ID}.Rout
rsync -av $TMP/$FOLDERNAME/results $RESULTDIR/

#Copy back to home directory
rsync -av $RESULTDIR ~/RSRF_CV/allfixed_seed/F6/results_F6
