...
prefered_order_algo <-  all_opt_names[ all_opt_names %in% unique( cv_result_dt$algorithm ) ]

cv_result_dt[ ,error_rounded := round(error,digits=3)]
cv_result_dt[ ,sd_rounded := round(sd,digits=3)]
cv_result_dt[, algorithm_factor := factor(algorithm, levels=prefered_order_algo)]
...
cv_result_dt[ , model_name := Vectorize(rename_man)(model) ]

#Order Table, first by model, then by d, then by order in all_opt_names below.
setorder(cv_result_dt, model, d, algorithm_factor )
...
for( model_id in models_chosen ){
  for( d_value in dimensions_chosen ){
    ...
    grid.table(  cv_result_dt[model == model_id & d==d_value,c("model_name", "d", "algorithm", "error_rounded", "sd_rounded")] )
    # table_g <- tableGrob( cv_result_dt[,c("model_name", "d", "error_rounded")] , rows=NULL, theme=ttheme_default(base_size=10))
    dev.off()
  }
}
